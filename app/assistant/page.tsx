...
export default function AssistantPage() {
  ...
  async function attachSurah(){
    ...
    try{
      const response=await fetch(`https://api.alquran.cloud/v1/surah/${surahNumber}/editions/quran-uthmani,fr.hamidullah`);
      ...
      setAttachment({label:`${surahs[surahNumber-1].name} · Sourate ${surahNumber}`,context});setPickerOpen(false);
    }catch{setError("Impossible de charger cette sourate pour le moment.")}finally{setLoading(false)}
  }
  ...
  async function send(text=draft,forcedAttachment=attachment){
    ...
    try{
      const response=await fetch("/api/ai-fiqh",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({messages:next.map(({role,content})=>({role,content})),attachment:forcedAttachment,safetyId:stableSafetyId()})});
      const data=await response.json();if(!response.ok)throw new Error(data.error||"Réponse indisponible.");
      setMessages(items=>[...items,{role:"assistant",content:data.answer}]);setAttachment(null);
    }catch(e){setError(e instanceof Error?e.message:"L’assistant est indisponible.")}finally{setLoading(false)}
  }
  ...
  return <main className="assistant-page">
    ...
    <section className="assistant-shell">
      <header className="assistant-hero"><span className="ai-mark">ن</span><div><small>ASSISTANT ÉDUCATIF</small><h1>Fqih</h1><p>Comprendre un passage, explorer une notion et distinguer les avis avec prudence.</p></div></header>
      ...
      <div className="chat-stream" aria-live="polite">
        ...
        {loading&&<article className="chat-message assistant thinking"><span>ن</span><div><i/><i/><i/></div></article>}
        {error&&<div className="ai-error">{error}</div>}<div ref={end}/>
      </div>
      <form className="chat-composer" onSubmit={submit}>
        {attachment&&<div className="pending-attachment">◈ {attachment.label}<button type="button" onClick={()=>setAttachment(null)}>×</button></div>}
        ...
        <div><button className="composer-plus" type="button" aria-label="Joindre une sourate" onClick={()=>setPickerOpen(v=>!v)}>+</button><textarea value={draft} onChange={e=>setDraft(e.target.value)} placeholder="Posez votre question…" rows={1}/><button className="composer-send" disabled={!draft.trim()||loading} aria-label="Envoyer">↑</button></div>
      </form>
    </section>
  </main>;
}
