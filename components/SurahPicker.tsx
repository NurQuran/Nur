...
export default function SurahPicker({ compact = false }: { compact?: boolean }) {
  ...
  function toggle(){const next=!open;setOpen(next);if(next)setTimeout(()=>{pickerRef.current?.scrollIntoView({behavior:"smooth",block:"start"});if(innerWidth<=900)setTimeout(()=>scrollBy({top:120,behavior:"smooth"}),180)},40)}
