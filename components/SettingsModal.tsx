...
export default function SettingsModal({value,onChange,onClose,studyPanel,customizePanel}:{value:ReaderOptions;onChange:(value:ReaderOptions)=>void;onClose:()=>void;studyPanel?:ReactNode;customizePanel?:ReactNode}){
  const {t,language}=useLanguage(); const [closing,setClosing]=useState(false),[tab,setTab]=useState<"settings"|"customize">("settings");
  ...
  return <div className={`modal-layer ${closing?"closing":""}`} role="presentation" onMouseDown={e=>e.target===e.currentTarget&&close()}>
    <section className="settings-modal" role="dialog" aria-modal="true" aria-labelledby="settings-title">
      ...
      <div className="settings-tabs"><button className={tab==="settings"?"active":""} onClick={()=>setTab("settings")}>Paramètres</button><button className={tab==="customize"?"active":""} onClick={()=>setTab("customize")}>Personnalisation</button></div>
      {tab==="settings"&&<>
      <div className="setting-group"><span className="setting-icon">♫</span><div><h3>{t("recitation")}</h3><p>{t("recitationHelp")}</p>
      ...
      {studyPanel}</>}
      {tab==="customize"&&<div className="customize-tab">{customizePanel}</div>}
