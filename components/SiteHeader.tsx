...
export default function SiteHeader({ active = "home", onSettings }: { active?: "home" | "read" | "favorites"; onSettings?: () => void }) {
  ...
  return <header className="topbar">
    ...
    <div className="header-actions">{onSettings && <button className="text-button settings-trigger" onClick={onSettings}><span aria-hidden="true">⌘</span> {t("settings")}</button>}<button className="icon theme-trigger" aria-label={theme === "dark" ? "Activer le mode clair" : "Activer le mode sombre"} onClick={toggleTheme}>{theme === "dark" ? "☀" : "☾"}</button></div>
  </header>;
}
