...
export default function SiteHeader({ active = "home", onSettings }: { active?: "home" | "read" | "favorites" | "assistant"; onSettings?: () => void }) {
  ...
  return <header className={`topbar${navHidden ? " mobile-hidden" : ""}`}>
    ...
    <div className="header-actions"><PwaInstallButton />{onSettings && <button className="text-button settings-trigger" onClick={onSettings} aria-label={t("settings")}><img className="header-png-icon" src="/icons/ui/settings.png" alt=""/><span className="settings-label">{t("settings")}</span></button>}<button className="icon theme-trigger" aria-label={theme === "dark" ? (language==="ar"?"تفعيل الوضع الفاتح":language==="en"?"Enable light mode":"Activer le mode clair") : (language==="ar"?"تفعيل الوضع الداكن":language==="en"?"Enable dark mode":"Activer le mode sombre")} onClick={toggleTheme}><img className="header-png-icon" src={theme === "dark" ? "/icons/ui/sun.png" : "/icons/ui/moon.png"} alt=""/></button></div>
  </header>;
}
