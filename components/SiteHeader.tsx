...
export default function SiteHeader({ active = "home", onSettings }: { active?: "home" | "read" | "favorites" | "assistant"; onSettings?: () => void }) {
  ...
  return <header className={`topbar${navHidden ? " mobile-hidden" : ""}`}>
    ...
    <div className="header-actions"><PwaInstallButton />{onSettings && <button className="text-button settings-trigger" onClick={onSettings}><span className="settings-glyph" aria-hidden="true"><i/><i/></span><span className="settings-label">{t("settings")}</span></button>}<button className="icon theme-trigger" aria-label={theme === "dark" ? "Activer le mode clair" : "Activer le mode sombre"} onClick={toggleTheme}><span className={theme === "dark" ? "theme-glyph sun-glyph" : "theme-glyph moon-glyph"} aria-hidden="true"><i/></span></button></div>
  </header>;
}
